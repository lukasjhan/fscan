...
fn main() {
    ...
    let size_format = match matches.is_present("raw-size") {
        true => fscan::print::SizeFormat::Raw,
        false => fscan::print::SizeFormat::Pretty,
    };
    let path = matches.value_of("DIRECTORY").unwrap();

    fscan::print::print_tree(
        &fscan::read_recursive(std::path::Path::new(&path), follow_symlinks),
        max_depth,
        size_format,
    );
}
