...
fn main() {
    let matches = clap::App::new(env!("CARGO_PKG_NAME"))
                            ...
                            .arg(Arg::with_name("size-format")
                                     ...
                                     .default_value("human")
                                     .takes_value(true)
                                     .possible_values(lib::print::SizeFormat::VALUES)
    ...
    let size_format = lib::print::SizeFormat::from_str(matches.value_of("size-format").unwrap()).unwrap();
    let path = matches.value_of("DIRECTORY").unwrap();

    lib::print::print_tree(&lib::read_recursive(std::path::Path::new(&path),
                                                            ignore_dotfiles,
                                                            follow_symlinks,
                                                            if one_file_system {
                                                              lib::filesystem::FilesystemBehaviour::OneFileSystemRoot
                                                            } else {
                                                              lib::filesystem::FilesystemBehaviour::Traverse
                                                            }),
